	...
	export default{
		...
		methods:{
			...
			async getcommpy(){
				...
				//获取计量单位数据
				let leaveurl="measures?token="+api.token+"&company_id="+this.company_id;
				...
				if(this.isedit==false){
					for(let i=0;i<this.lists.length;i++){
						...
						if(index==i){
							...
							this.$emit('my-measure',val,this.Refmeasure)
						}
					}
				}else{
					for(let i=0;i<this.lists.length;i++){
						...
						if(index==i){
							Vue.set(this.lists[i],'istrue','true')
							// 跳转修改页面
							this.$router.push({name:"Measureedit",params:{obj:val}})
						}
					}
				}
			...
			// 跳转添加页面
			add(){
				this.$router.push({name:"Measureadd",params:{company_id:this.company_id,company_name:this.company_name,app_user_id:this.app_user_id}})
			},
			completes(){
				this.isedit=false
				this.getcommpy()
