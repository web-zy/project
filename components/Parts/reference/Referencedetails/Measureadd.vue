<template>
	<div  @click="hide">
		<div v-if="isbackground" class="posback"></div>
		<von-header>
		...
		<div class="reference reference-one">
			<p class="required">*</p>
			<p>计量单位</p>
			<input type="text" v-model="name" placeholder="请输入要添加的计量单位"/>
		</div>
		<div class="reference">
			<p>备注</p>
...
<script>
	...
	export default{
		...
		mounted(){
			...
			this.user_id=this.$route.params.app_user_id
		...
		methods:{
			...
			async addSubmit(){
				let _this=this
				if(this.name==""){
					$toast.show("计量单位不能为空", 1000).then(() => {})
				}else{
					let url="measures?bizid="+this.bizid+"&token="+api.token;
