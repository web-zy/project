	...
	export default{
		...
		methods:{
			async getcommpy(){
				...
				//获取借款类型数据
				let leaveurl="loantypes?token="+api.token+"&company_id="+this.company_id+"&size=30";
			...
			complete(val,index){
				...
				// 判断是修改的对号还是添加的对号
				if(val.id){					//val.id是独有的id
					console.log("修改接口")
					// 修改接口

					let url="loantypes/"+id+"/?bizid="+bizid+"&token="+api.token
					let res=api.put(url,{
						name:val.name,
					...
					if(val.name==""){
						...
						// 添加接口
						let url="loantypes?bizid="+bizid+"&token="+api.token;
