            ...
            <div style="margin-bottom:0.25rem" v-for="(val,index) in detailArrList" :key="index">
              <item v-for="item in val">
                <div class="info-list">
                  ...
                  <span v-if="item.sing == 'in_amt'">{{item.content | formatNum}}</span>
                  <span v-else-if="item.sing == 'fundtransfer_date'">{{item.content | time}}</span>
  ...
  export default {
    data() {
      return {
        ...
        detailArrList:[],
        detailsList:[
          {name:"资金划拨单号",content:"",sing:"fundtransfer_code"},
          {name:"划拨日期",content:"",sing:"fundtransfer_date"},
          {name:"转入银行",content:"",sing:"inbank_name"},
          {name:"转入账号",content:"",sing:"inaccount_name"},
          {name:"转入金额",content:"",sing:"in_amt"},
    ...
    methods: {
      ...
      async getDelData(){
        ...
        // let url = `fafundcancels?token=${api.token}&id=${id}&name=${this.del_info.name}`;
        let res = await api.get(url);
        if(res.status >= 200 && res.status < 300){
          res = res.data;
          if(res.length === 0){
            $dialog.alert({title:"提示",content:"您没有资金划拨单，无法操作",okText:"返回",theme:"ios"})
