            ...
            <div style="margin-bottom:0.25rem" v-for="(val,index) in detailArrList" :key="index" >
              <item v-for="item in val">
                <div class="info-list">
                  ...
                  <span v-if="item.sing == 'send_date'">{{item.content | time}}</span>
                  <span v-else-if="item.sing == 'sales_amt'">{{item.content | formatNum}}</span>
  ...
  export default {
    data() {
      return {
        ...
        detailArrList:[],
        detailsList:[
          {name:"出库单号",content:"",sing:"outstorage_code"},
          {name:"客户订单号",content:"",sing:"saleorder_code"},
          {name:"发货日期",content:"",sing:"send_date"},
          {name:"业务员",content:"",sing:"salesman_name"},
          {name:"收货单位",content:"",sing:"customer_name"},
          {name:"结算总金额",content:"",sing:"sales_amt"},
          {name:"本次销账金额",content:"",sing:"outstcancel_amt"},
    ...
    methods: {
      ...
      async getDelData(){
        ...
        let url = `faoutstcancels?token=${api.token}&filter=TaskID=${id}`;
