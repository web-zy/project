        ...
        <div class="subInfo">
        	...
        	<div class="info">
        		...
        		<div class="a-single">
        			...
        			<span>：¥{{orderData.total_receivable_amt | formatNum}}</span>
        		</div>
        	</div>
        </div>
        <div class="tabsWarp">
        				...
        				<item v-for="item in val">
							<div class="info-list">
								...
								<!-- <span>{{item.content}}</span> -->
								 <span v-if="item.sing == 'business_date'">{{item.content | time}}</span>
	...
	export default {
		data() {
			return {
				...
				details:[],
				detailsList:[
					{name:"客户名称",content:"",sing:"customer_name"},
					{name:"业务日期",content:"",sing:"business_date"},
					{name:"应收金额",content:"",sing:"receivable_amt"},
		...
		methods: {
			...
			async getDelData(){
				...
				//应收期初信息
				this.details=res[0].qc_receivable_details_t;
				this.details = utrl.setJson(this.details,this.detailsList);
				utrl.thousand(this.details,"receivable_amt")
			},
