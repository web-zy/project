        ...
        <div class="subInfo">
        	...
        	<div class="info">
        		...
        		<div class="a-single">
        			...
        			<span>：¥{{orderData.total_pay_amt | formatNum}}</span>
        		</div>
        	</div>
        </div>
        <div class="tabsWarp">
	...
	export default {
		data() {
			return {
				...
				details:[],
				detailsList:[
					{name:"供应商名称",content:"",sing:"supplier_name"},
					{name:"业务日期",content:"",sing:"business_date"},
					{name:"应付金额",content:"",sing:"pay_amt"},
		...
		methods: {
			...
			async getDelData(){
				...
				//应付期初信息
				this.details=res[0].qc_payable_details_t;
				this.details = utrl.setJson(this.details,this.detailsList);
				utrl.thousand(this.details,"pay_amt")
			},
