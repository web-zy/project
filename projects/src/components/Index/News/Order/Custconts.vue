	        ...
	        <div class="subInfo">
	        	...
	        	<div class="info">
	        		...
	        		<div class="a-single">
	        			...
	        			<span>：{{orderData.total_qty | formatNum}}件  ¥{{orderData.total_bd_amt | formatNum}}</span> -->
						...
						<div class="detailed-list" v-for="(val,index) in custcontslist" :key="index" @click="showDetailed(index)">
							<div class="brief-warp">
								...
								<div class="detailed-info">
									...
									<span>{{val.topobj.brand_name}} {{val.topobj.remarks}} </span>
							...
							<div class="drop-down" id="drop-down1" style="padding:0" :class="val.showDrop ? 'show-this' : 'hide-this'">
		...
		methods: {
			...
			setTrialList(v){
				...
				for(var i=0;i<this.custcontslist.length;i++){
					...
					let index = 0;
					let removeIndex = 0;
					for(let j in this.custcontslist[i]){
						if(this.custcontslist[i][j].sing == 'measure_name'){
							tmpName = this.custcontslist[i][j].content;
							removeIndex = j;
						}
						if(this.custcontslist[i][j].sing == "qty"){
							index = j;
						}
					}
					this.custcontslist[i][index].content = this.custcontslist[i][index].content + tmpName
					// 删除单位
					this.custcontslist[i].splice(removeIndex,1)
					// 添加数据
					brr.push({
						topobj:arr[i],
